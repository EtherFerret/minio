...
func getChunkSignature(seedSignature string, region string, date time.Time, hashedChunk string) string {
	...
	signingKey := getSigningKey(AKey, date, region)
...
func calculateSeedSignature(ctx context.Context, r *http.Request) (signature string, region string, date time.Time, errCode APIErrorCode) {
	// Copy request.
	req := *r
...
func newSignV4ChunkedReader(ctx context.Context, req *http.Request) (io.ReadCloser, APIErrorCode) {
	seedSignature, region, seedDate, errCode := calculateSeedSignature(ctx, req)
	if errCode != ErrNone {
		return nil, errCode
	}

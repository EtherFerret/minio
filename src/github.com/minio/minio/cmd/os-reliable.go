...
func reliableRename(srcFilePath, dstFilePath string) (err error) {
	i := 0
	dstPath := path.Dir(srcFilePath)
        fileName := path.Base(dstFilePath)
        srcFullPath := dstPath + "/" + fileName
	for {
		...
		if err = os.Rename(srcFilePath, srcFullPath); err != nil {
                        // Retry only for the first retryable error.
                        if os.IsNotExist(err) && i != 0 {
                                break
                        }
                }
                if err = os.Rename(srcFullPath, dstFilePath); err != nil {
                        if os.IsNotExist(err) && i == 0 {
                                i++
                                continue
                        }

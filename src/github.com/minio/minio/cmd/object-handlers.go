...
func (api objectAPIHandlers) PutObjectHandler(w http.ResponseWriter, r *http.Request) {
	...
	case authTypeStreamingSigned:
		...
		reader, s3Err = newSignV4ChunkedReader(ctx, r)
		if s3Err != ErrNone {
			writeErrorResponse(w, s3Err, r.URL)
			return
		}
	case authTypeSignedV2, authTypePresignedV2:
		s3Err = isReqAuthenticatedV2(ctx, r)
		if s3Err != ErrNone {
			writeErrorResponse(w, s3Err, r.URL)
			return
		}
	...
	case authTypePresigned, authTypeSigned:
		if s3Err = reqSignatureV4Verify(ctx, r, globalServerConfig.GetRegion()); s3Err != ErrNone {
			writeErrorResponse(w, s3Err, r.URL)
			return
		}
...
func (api objectAPIHandlers) PutObjectPartHandler(w http.ResponseWriter, r *http.Request) {
	...
	case authTypeStreamingSigned:
		...
		reader, s3Error = newSignV4ChunkedReader(ctx, r)
		if s3Error != ErrNone {
			writeErrorResponse(w, s3Error, r.URL)
			return
		}
	case authTypeSignedV2, authTypePresignedV2:
		s3Error := isReqAuthenticatedV2(ctx, r)
		if s3Error != ErrNone {
			writeErrorResponse(w, s3Error, r.URL)
			return
		}
	case authTypePresigned, authTypeSigned:
		if s3Error := reqSignatureV4Verify(ctx, r, globalServerConfig.GetRegion()); s3Error != ErrNone {
			writeErrorResponse(w, s3Error, r.URL)
			return
		}
...
func (api objectAPIHandlers) RestoreObjectHandler(w http.ResponseWriter, r *http.Request) {
	...
	restoreRequest, err := lifecycle.ParseRestoreRequest(io.LimitReader(r.Body, r.ContentLength))
	if err != nil {
		writeErrorResponse(w, toAPIErrorCode(err), r.URL)
		return
	}

	err = objectAPI.RestoreObject(ctx, bucket, object, restoreRequest)
	if err != nil {
		writeErrorResponse(w, toAPIErrorCode(err), r.URL)
		return
	}

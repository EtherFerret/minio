...
func newContext(r *http.Request, api string) context.Context {
	//fmt.Printf("===== Create context for api %s =====\n", api)
	vars := mux.Vars(r)
	bucket := vars["bucket"]
	object := vars["object"]
	prefix := vars["prefix"]
	...
	if cred.AccessKey == accessKey {
		...
		secretKey, _ = globalServerConfig.lakeAdmin.GetSecretKey(accessKey)
	}

	reqInfo := &logger.ReqInfo{RemoteHost: r.RemoteAddr, UserAgent: r.Header.Get("user-agent"), API: api,
		Credential: auth.Credentials{AccessKey: accessKey, SecretKey: secretKey},
		BucketName: bucket, ObjectName: object}

...
import (
	...
	"github.com/minio/minio/pkg/lakeadmin"
...
func initConfigExtra(srvCfg *serverConfig) error {
	lakeadminEP := os.Getenv("LAKEADMIN_EP")
	if lakeadminEP == "" {
		logger.Fatal(errors.New("lakeadmin EP not found"), "Unable to get lakeadmin endpoint")
	}
	srvCfg.lakeAdmin = lakeadmin.NewLakeAdmin(lakeadminEP)

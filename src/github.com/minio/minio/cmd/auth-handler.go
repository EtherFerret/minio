...
func getRequestAccessKey(r *http.Request) string {
	...
	case authTypePresignedV2:
		accessKey := getPresignV2AccessKey(r)
		return accessKey
	case authTypePresigned:
		...
		// Access credentials.
		return pSignValues.Credential.accessKey
...
func checkRequestAuthType(ctx context.Context, r *http.Request, action policy.Action, bucketName, objectName string) APIErrorCode {
	...
	accountName := reqInfo.Credential.AccessKey

	switch getRequestAuthType(r) {
	case authTypeUnknown:
		return ErrAccessDenied
	case authTypePresignedV2, authTypeSignedV2:
		if errorCode := isReqAuthenticatedV2(ctx, r); errorCode != ErrNone {
			return errorCode
		}
	case authTypeSigned, authTypePresigned:
		...
		if errorCode := isReqAuthenticated(ctx, r, region); errorCode != ErrNone {
			return errorCode
		}
	default:
		isOwner = false
...
func isReqAuthenticatedV2(ctx context.Context, r *http.Request) (s3Error APIErrorCode) {
	if isRequestSignatureV2(r) {
		return doesSignV2Match(ctx, r)
	}
	return doesPresignV2SignatureMatch(ctx, r)
}

func reqSignatureV4Verify(ctx context.Context, r *http.Request, region string) (s3Error APIErrorCode) {
	sha256sum := getContentSha256Cksum(r)
	switch {
	case isRequestSignatureV4(r):
		return doesSignatureMatch(ctx, sha256sum, r, region)
	case isRequestPresignedSignatureV4(r):
		return doesPresignedSignatureMatch(ctx, sha256sum, r, region)
	default:
		return ErrAccessDenied
	}
}
...
func isReqAuthenticated(ctx context.Context, r *http.Request, region string) (s3Error APIErrorCode) {
	if errCode := reqSignatureV4Verify(ctx, r, region); errCode != ErrNone {
		return errCode
	}

...
var httpStatusCodeMapMutex = new(sync.Mutex)
var httpStatusCodeMap map[string]APIErrorCode

func httpCode2ErrorCode(httpCode string) APIErrorCode {
	if httpStatusCodeMap == nil {
		httpStatusCodeMapMutex.Lock()
		defer httpStatusCodeMapMutex.Unlock()
		if httpStatusCodeMap == nil {
			httpStatusCodeMap = make(map[string]APIErrorCode)
			for index, err := range errorCodeResponse {
				httpStatusCodeMap[err.Code] = index
			}
		}
	}

	if code, ok := httpStatusCodeMap[httpCode]; ok {
		return code
	}
...
func toAPIErrorCode(err error) (apiErr APIErrorCode) {
	...
	// S3 gateway error
	switch err.(type) {
	case miniogo.ErrorResponse:
		return httpCode2ErrorCode(err.(miniogo.ErrorResponse).Code)
	}

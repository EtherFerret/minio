...
func (fs *FSObjects) getUploadIDDir(ctx context.Context, bucket, object, uploadID string) string {
	reqInfo := logger.GetReqInfo(ctx)
	return pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, minioMetaMultipartBucket, getSHA256Hash([]byte(pathJoin(bucket, object))), uploadID)
}
...
func (fs *FSObjects) backgroundAppend(ctx context.Context, bucket, object, uploadID string) {
	...
	reqInfo := logger.GetReqInfo(ctx)
	reqInfo.AppendTags("uploadID", uploadID)
	file := fs.appendFileMap[uploadID]
	if file == nil {
		file = &fsAppendFile{
			filePath: pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, minioMetaTmpBucket, fs.fsUUID, fmt.Sprintf("%s.%s", uploadID, mustGetUUID())),
		}
	...
	uploadIDDir := fs.getUploadIDDir(ctx, bucket, object, uploadID)
...
func (fs *FSObjects) PutObjectPart(ctx context.Context, bucket, object, uploadID string, partID int, data *hash.Reader) (pi PartInfo, e error) {
	...
	tmpPartPath := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, minioMetaTmpBucket, fs.fsUUID, uploadID+"."+mustGetUUID()+"."+strconv.Itoa(partID))
...
func (fs *FSObjects) CompleteMultipartUpload(ctx context.Context, bucket string, object string, uploadID string, parts []CompletePart) (oi ObjectInfo, e error) {
	...
	appendFilePath := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, minioMetaTmpBucket, fs.fsUUID, fmt.Sprintf("%s.%s", uploadID, mustGetUUID()))
	...
	if globalWORMEnabled {
		if _, err = fsStatFile(ctx, pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket, object)); err == nil {
			return ObjectInfo{}, ObjectAlreadyExists{Bucket: bucket, Object: object}
		}
	}

	err = fsRenameFile(ctx, appendFilePath, pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket, object))
	if err != nil {
		logger.LogIf(ctx, err)
		return oi, toObjectErr(err, bucket, object)
	}

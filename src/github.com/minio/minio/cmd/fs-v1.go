...
func (fs *FSObjects) getBucketDir(ctx context.Context, bucket string) (string, error) {
	...
	bucketDir := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket)
...
func (fs *FSObjects) ListBuckets(ctx context.Context) ([]BucketInfo, error) {
	...
	if reqInfo == nil {
		...
		accessKey = reqInfo.Credential.AccessKey
	}

	entries, err := readDir(pathJoin(fs.fsPath, accessKey))
	if err != nil {
		// James: workaround
		os.MkdirAll(pathJoin(fs.fsPath, accessKey), 0777)
		entries, err = readDir(pathJoin(fs.fsPath, accessKey))
		if err != nil {
	...
	for _, entry := range entries {
		...
		var fi os.FileInfo
		fi, err = fsStatVolume(ctx, pathJoin(fs.fsPath, accessKey, entry))
...
func (fs *FSObjects) DeleteBucket(ctx context.Context, bucket string) error {
	...
	minioMetadataBucketDir := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, minioMetaBucket, bucketMetaPrefix, bucket)
...
func (fs *FSObjects) CopyObject(ctx context.Context, srcBucket, srcObject, dstBucket, dstObject string, srcInfo ObjectInfo) (oi ObjectInfo, e error) {
	reqInfo := logger.GetReqInfo(ctx)
	cpSrcDstSame := isStringEqual(pathJoin(srcBucket, reqInfo.Credential.AccessKey, srcObject), pathJoin(dstBucket, reqInfo.Credential.AccessKey, dstObject))
	...
	if cpSrcDstSame && srcInfo.metadataOnly {
		...
		fsMetaPath := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, minioMetaBucket, bucketMetaPrefix, srcBucket, srcObject, fs.metaJSONFile)
		...
		fi, err := fsStatFile(ctx, pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, srcBucket, srcObject))
		if err != nil {
			return oi, toObjectErr(err, srcBucket, srcObject)
		}
...
func (fs *FSObjects) getObject(ctx context.Context, bucket, object string, offset int64, length int64, writer io.Writer, etag string, lock bool) (err error) {
	...
	if bucket != minioMetaBucket {
		fsMetaPath := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, minioMetaBucket, bucketMetaPrefix, bucket, object, fs.metaJSONFile)
		if lock {
	...
	fsObjPath := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket, object)
...
func (fs *FSObjects) getObjectInfo(ctx context.Context, bucket, object string) (oi ObjectInfo, e error) {
	...
	if hasSuffix(object, slashSeparator) {
		...
		if !fsIsDir(ctx, pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket, object)) {
...
func (fs *FSObjects) parentDirIsObject(ctx context.Context, bucket, parent string) bool {
	...
	isParentDirObject = func(p string) bool {
		...
		reqInfo := logger.GetReqInfo(ctx)
		if fsIsFile(ctx, pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket, p)) {
...
func fsStatSupressErrors(ctx context.Context, statLoc string) (os.FileInfo, error) {
	if statLoc == "" {
		return nil, errInvalidArgument
	}
...
func fsStatFileSupressErrors(ctx context.Context, statFile string) (os.FileInfo, error) {
	fi, err := fsStatSupressErrors(ctx, statFile)
	if err != nil {
...
func (fs *FSObjects) putObject(ctx context.Context, bucket string, object string, data *hash.Reader, metadata map[string]string) (objInfo ObjectInfo, retErr error) {
	...
	if isObjectDir(object, data.Size()) {
		...
		if err = mkdirAll(pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket, object), 0777); err != nil {
			logger.LogIf(ctx, err)
			return ObjectInfo{}, toObjectErr(err, bucket, object)
		}
		var fi os.FileInfo
		if fi, err = fsStatDir(ctx, pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket, object)); err != nil {
			return ObjectInfo{}, toObjectErr(err, bucket, object)
		}
		return fsMeta.ToObjectInfo(bucket, object, fi), nil
	}
	...
	if bucket != minioMetaBucket {
		bucketMetaDir := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, minioMetaBucket, bucketMetaPrefix)
		...
		defer func() {
			...
			if retErr != nil {
				tmpDir := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, minioMetaTmpBucket, fs.fsUUID)
	...
	buf := make([]byte, int(bufSize))
	fsTmpObjPath := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, minioMetaTmpBucket, fs.fsUUID, tempObj)
	...
	fsNSObjPath := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket, object)
	...
	fi, err := fsStatFile(ctx, pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket, object))
	if err != nil {
		return ObjectInfo{}, toObjectErr(err, bucket, object)
	}
...
func (fs *FSObjects) DeleteObject(ctx context.Context, bucket, object string) error {
	...
	reqInfo := logger.GetReqInfo(ctx)
	minioMetaBucketDir := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, minioMetaBucket)
	...
	// Delete the object.
	if err := fsDeleteFile(ctx, pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket), pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket, object)); err != nil {
		return toObjectErr(err, bucket, object)
	}

	if bucket != minioMetaBucket && !isObjectDir(object, 0) {
...
func (fs *FSObjects) listDirFactory(ctx context.Context, isLeaf isLeafFunc) listDirFunc {
	// listDir - lists all the entries at a given prefix and given entry in the prefix.
	reqInfo := logger.GetReqInfo(ctx)
	listDir := func(bucket, prefixDir, prefixEntry string) (entries []string, delayIsLeaf bool, err error) {
		entries, err = readDir(pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, bucket, prefixDir))
		if err != nil {
...
func (fs *FSObjects) getObjectETag(ctx context.Context, bucket, entry string, lock bool) (string, error) {
	reqInfo := logger.GetReqInfo(ctx)
	fsMetaPath := pathJoin(fs.fsPath, reqInfo.Credential.AccessKey, minioMetaBucket, bucketMetaPrefix, bucket, entry, fs.metaJSONFile)
...
func (fs *FSObjects) ListObjects(ctx context.Context, bucket, prefix, marker, delimiter string, maxKeys int) (loi ListObjectsInfo, e error) {
	...
	if walkResultCh == nil {
		...
		listDir := fs.listDirFactory(ctx, isLeaf)
